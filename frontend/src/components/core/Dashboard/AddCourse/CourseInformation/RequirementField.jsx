    ...
    <div className="flex flex-col space-y-2">
      ...
      <div className="flex flex-col items-start space-y-2">
        ...
        <button
          ...
          className="font-semibold text-yellow-600 dark:text-yellow-50"
        >
          Add
        </button>
      </div>
      ...
      {requirementsList.length > 0 && (
        <ul className="mt-2 list-inside list-disc">
          {requirementsList.map((requirement, index) => (
            <li key={index} className="flex items-center text-richblack-900 dark:text-richblack-5">
              <span>{requirement}</span>
              <button
                type="button"
                className="ml-2 text-xs text-pure-greys-500 dark:text-pure-greys-300 "
